...
def publisher(server="127.0.0.1:7111", timecode=False):
    # Setup Client
    with pyrtma.client_context(server_name=server, timecode=timecode) as mod:
...
def subscriber(server="127.0.0.1:7111", timecode=False):
    # Setup Client and select the messages to receive
    with pyrtma.client_context(
        server_name=server,
        timecode=timecode,
        msg_list=[md.MT_PERSON_MESSAGE, md.MT_EXIT],
    ) as mod:
